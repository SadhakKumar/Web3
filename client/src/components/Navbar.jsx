    ...
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      ...
      <div className='md:hidden flex justify-between items-center relative'>
        <div className='w-[40px] h-[40px] rounded-[10px] bg-[#2c2f32] flex justify-center items-center cursor-pointer'>
          <img src={thirdweb} alt="thirdweb" className='w-[60%] h-[60%] objects-contain' />
        </div>

        <img src={menu} alt="menu" className='w-[34px] h-[34px] object-contain cursor-pointer' onClick={() => setToggleDrawer(!toggleDrawer)} />

        <div className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-10 shadow-secondary py-4 ${!toggleDrawer ? '-translate-y-[150vh]' : '-translate-y-0'} translate-all duration-700`}>
          <ul className='mb-4'>
            {navlinks.map((link) => (
              <li
                key={link.name}
